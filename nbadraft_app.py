...
if st.session_state.get('draft_started'):
    ...
    if not all_rosters_full(rosters):
        ...
        available_players = [p for p in st.session_state.game_state['available_players']
                             if p not in st.session_state.game_state['drafted_players']]
        if not available_players:
            st.error("No more available NBA players.")
            st.stop()

        if 'current_nba_player' not in st.session_state:
            st.session_state.current_nba_player = random.choice(available_players)

        current_nba_player = st.session_state.current_nba_player
        stats = st.session_state['player_stats'][current_nba_player]
        ...
        if st.button("✅ Submit Bid"):
            if winning_bidder == "Skip":
                ...
                st.error("❗ Select a valid roster spot for the winning bidder!")
            else:
